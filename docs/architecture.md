...
| 组件 | 主要职责 | 代码入口 |
| --- | --- | --- |
| 进程入口 | 加载 bundled dependencies、读取配置、清理临时状态、启动后台任务和 HTTP 服务 | `embyToLocalPlayer.py` |
| 本地 HTTP 服务 | 精确路由、协议头/令牌校验、请求体解析、播放/文件/缓存动作分派和短期媒体 URL | `utils/http_server.py`、`utils/http_security.py` |
...
- 进程启动时由 `embyToLocalPlayer.py` 清理临时状态，准备配置，启动恢复信息预取
