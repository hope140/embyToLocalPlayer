...
Knowledge Findings: 新约束、新坑、被证明错误的旧假设、建议沉淀项、是否需更新架构文档/ADR（无则写“无”）
```

主线程不得用模糊任务替代上述字段。代理报告边界不清、冲突或架构问题后，主线程先修订计划和契约再继续。

Knowledge Findings 只能写由真实代码、测试、实际运行/日志、可复现结果，或官方文档与当前实现交叉确认的事实；项目文档单独陈述不足以作为事实门槛。写入知识库前必须搜索去重，旧经验失效时更新当前事实。
...
7. 对具有实质性代码修改、Bug、架构或兼容性调查的任务完成 Knowledge Review：搜索现有知识避免重复，更新失效事实；仅沉淀稳定、可复用且已验证的结论，任务日志和敏感本机信息不得进入知识库。无发现时在报告写明 `Knowledge Findings/Review: 无`，不强行修改知识文档。
