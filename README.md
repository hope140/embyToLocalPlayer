# etlp - embyToLocalPlayer（CloudDrive2 实验分支）

etlp - Emby/Jellyfin 调用 PotPlayer mpv IINA MPC VLC 播放，并回传播放进度（可关）。适配 Plex。

本分支用于实验将本地挂载路径解析/访问切换为 CloudDrive2。分支只保留播放器、路径转换、STRM、本地预热和进度回传等主线能力；不包含同步观看、豆瓣/Bangumi、Simkl/Trakt、聚合搜索和 qBittorrent 联动等旁线功能。
